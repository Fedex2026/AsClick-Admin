...
let unsubscribeProviderLocations = null;
...
let liveProviderLayer = null;

 

let fullProviderLayer = null;
...
function datosIconoProveedorMapa(proveedor){

 

  const tipo = normalizarTipoServicio(proveedor?.raw?.tipoProveedor || proveedor?.raw?.tipo || proveedor?.type || "");

 

  if (tipo === "Grúa") return { simbolo:"🚚", color:"#f26a21", tipo };

 

  if (tipo === "Ajustador") return { simbolo:"⚖", color:"#8d42dc", tipo };

 

  if (tipo === "Abogado") return { simbolo:"⚖", color:"#1aa6a3", tipo };

 

  if (tipo === "Auxilio Vial") return { simbolo:"🚗", color:"#278b43", tipo };

 

  return { simbolo:"●", color:"#1579d0", tipo:tipo || "Proveedor" };
...
function obtenerCoordenadasProveedorMapa(ubicacion = {}){
...
function proveedorVisibleEnMapa(proveedor,ubicacion = {}){

 

  if (!proveedor || proveedorEstaDeBaja(proveedor)) return false;

 

  const estado = estadoProveedorEnTiempoReal(proveedor);

 

  const servicioActualId = ubicacion.servicioActualId || proveedor?.raw?.servicioActualId || "";

 

  return estado === "Disponible" || estado === "Ocupado" || Boolean(servicioActualId);
...
function popupProveedorMapa(proveedor,ubicacion = {}){
  ...
  const icono = datosIconoProveedorMapa(proveedor);

 

  const actualizado = ubicacion.actualizadoEn || ubicacion.ultimaActualizacion || proveedor?.raw?.ultimaActualizacion || null;

 

  const actualizadoTexto = actualizado ? tiempoRelativo(actualizado) : "Sin actualización";
  ...
  return `<b>${escaparHtml(proveedor.name)}</b><br>${escaparHtml(icono.tipo)}<br>Estado: ${escaparHtml(estado)}<br>Tel: ${escaparHtml(proveedor.telefono)}<br>Ubicación: ${escaparHtml(actualizadoTexto)}${servicioActualId ? `<br>Servicio: ${escaparHtml(servicioActualId)}` : ""}`;
...
function renderProviderLocationsOnMap(map,layer){
  ...
  state.providerLocations.forEach((ubicacion,uid) => {

 

    const proveedor = obtenerProveedorPorUid(uid);

 

    const coords = obtenerCoordenadasProveedorMapa(ubicacion);

 

    if (!proveedor || !coords || !proveedorVisibleEnMapa(proveedor,ubicacion)) return;
    ...
    L.marker(coords,{icon:makeDivIcon(icono.simbolo,icono.color)}).addTo(layer).bindPopup(popupProveedorMapa(proveedor,ubicacion));
...
function renderProviderLocations(){

 

  renderProviderLocationsOnMap(liveMap,liveProviderLayer);

 

  renderProviderLocationsOnMap(fullMap,fullProviderLayer);
...
function initMaps(){
  ...
  if (typeof L === "undefined" || liveMap) return;
  ...
  liveProviderLayer = L.layerGroup().addTo(liveMap);
...
function initFullMap(){
  ...
  fullProviderLayer = L.layerGroup().addTo(fullMap);
...
function reportFechaServicio(servicio){
  ...
  return obtenerMilisegundos(
    ...
    servicio?.raw?.creadoEn ||

 

    servicio?.raw?.fechaCreacion ||

 

    servicio?.raw?.fechaSolicitud
...
function reportCanalServicio(servicio){

 

  const d = servicio?.raw || {};
  ...
  return tieneMembresia ? "Membresía" : "Particular";
...
function reportCalificacionServicio(servicio){
  ...
  const valores = [
    ...
    d.encuesta?.calificacion,

 

    d.calificacionServicio
  ...
  const numero = valores.map(Number).find(v => Number.isFinite(v) && v > 0);
...
function reportMinutosLlegada(servicio){
  ...
  const inicio = obtenerMilisegundos(
  ...
  const llegada = obtenerMilisegundos(
  ...
  if (!inicio || !llegada || llegada < inicio) return 0;

 

  return Math.round((llegada - inicio) / 60000);
...
function obtenerRangoReportes(){
  ...
  const desdeInput = document.getElementById("reportDateFrom")?.value || "";
  ...
  return {
    ...
    desdeMs: desde && !Number.isNaN(desde.getTime()) ? desde.getTime() : 0,

 

    hastaMs: hasta && !Number.isNaN(hasta.getTime()) ? hasta.getTime() : Number.MAX_SAFE_INTEGER
...
function obtenerServiciosReporte(){

 

  const {desdeMs,hastaMs} = obtenerRangoReportes();

 

  const tipo = document.getElementById("reportTypeFilter")?.value || "";

 

  const estado = document.getElementById("reportStatusFilter")?.value || "";

 

  const canal = document.getElementById("reportChannelFilter")?.value || "";

 

  return state.services.filter(servicio => {

 

    const fechaMs = reportFechaServicio(servicio);

 

    if (!fechaMs || fechaMs < desdeMs || fechaMs > hastaMs) return false;

 

    if (tipo && servicio.servicio !== tipo) return false;
    ...
    if (canal && reportCanalServicio(servicio) !== canal) return false;
...
function aplicarRangoRapidoReportes(rango = "30"){
  ...
  if (rango === "today") {
    ...
    const dias = Math.max(1,Number(rango) || 30);
  ...
  const desdeEl = document.getElementById("reportDateFrom");

 

  const hastaEl = document.getElementById("reportDateTo");

 

  if (desdeEl) desdeEl.value = reportFechaLocalInput(desde);

 

  if (hastaEl) hastaEl.value = reportFechaLocalInput(hasta);

 

  document.querySelectorAll(".reportRangeBtn").forEach(btn => {

 

    btn.classList.toggle("active",btn.dataset.reportRange === String(rango));
...
function reportAgrupar(lista,obtenerClave){
  ...
  return lista.reduce((map,item) => {
    ...
    const key = obtenerClave(item) || "Sin dato";
...
function renderReportBreakdown(id,map,total){
  ...
  if (!items.length) {
    ...
    el.innerHTML = `<div class="reportEmpty">Sin datos para el periodo seleccionado.</div>`;
  ...
  el.innerHTML = items.map(([nombre,valor]) => {

 

    const porcentajeTotal = total ? Math.round((valor / total) * 100) : 0;

 

    const ancho = Math.max(3,Math.round((valor / max) * 100));
    ...
    return `
      ...
      <div class="reportBreakdownRow">
        ...
        <div class="reportBreakdownTrack">

 

          <div class="reportBreakdownFill" style="width:${ancho}%"></div>
        ...
        <span>${valor} · ${porcentajeTotal}%</span>
...
function reportDibujarBarras(canvasId,datos){
  ...
  const slot = usableW / datos.length;

 

  const barW = Math.max(5,Math.min(28,slot*.58));
  ...
  datos.forEach((d,i) => {
    ...
    const bh = (d.valor/max)*(h-padT-padB);
    ...
    ctx.rotate(datos.length > 12 ? -0.55 : 0);
...
function reportDibujarTipos(canvasId,map){
  ...
  if (!items.length) {
    ...
    ctx.fillText("Sin datos para el periodo seleccionado.",30,45);
  ...
  items.forEach(([nombre,valor],i) => {
    ...
    ctx.fillText(nombre,left-12,y+15);
    ...
    ctx.fillText(String(valor),left+width+8,y+15);
...
function renderReportProviders(servicios){

 

  const body = document.getElementById("reportProvidersBody");

 

  const count = document.getElementById("reportProviderCount");
  ...
  servicios.forEach(s => {
    ...
    const uid = String(s.uidProveedor || s.raw?.asignacion?.uidProveedor || "").trim();

 

    const nombre = s.proveedor || "Sin asignar";

 

    const key = uid || `nombre:${nombre}`;
    ...
    if (!mapa.has(key)) {
      ...
      const proveedor = uid ? obtenerProveedorPorUid(uid) : state.providers.find(p => p.name === nombre);
      ...
      mapa.set(key,{
        ...
        tipo: proveedor?.type || s.servicio || "—",
    ...
    if (s.estado === "Finalizado") {
      ...
      item.ingresos += Number(s.monto) || 0;
    ...
    if (s.estado === "Cancelado") item.cancelados += 1;

 

    const rating = reportCalificacionServicio(s);

 

    if (rating) item.ratings.push(rating);
  ...
  if (count) count.textContent = `${filas.length} ${filas.length === 1 ? "proveedor" : "proveedores"}`;
  ...
  body.innerHTML = filas.length
    ...
    ? filas.map(p => {
        ...
        return `
          ...
          <tr>
            ...
            <td><b>${escaparHtml(p.nombre)}</b></td>
            ...
            <td>${escaparHtml(formatearDinero(p.ingresos))}</td>
    ...
    : `<tr><td colspan="7">No hay proveedores con servicios en el periodo seleccionado.</td></tr>`;
...
function renderReports(){
  ...
  const contenedor = document.getElementById("reportKpis");
  ...
  const servicios = obtenerServiciosReporte();
  ...
  const enCurso = servicios.filter(s => ["Asignado","En camino","En sitio"].includes(s.estado));

 

  const ingresos = finalizados.reduce((sum,s)=>sum+(Number(s.monto)||0),0);

 

  const ticket = finalizados.length ? ingresos/finalizados.length : 0;

 

  const tiempos = servicios.map(reportMinutosLlegada).filter(v => v > 0 && v < 24*60);
  ...
  const kpis = [
    ...
    ["Servicios",total.toLocaleString("es-MX"),"Total del periodo","▣"],

 

    ["Finalizados",finalizados.length.toLocaleString("es-MX"),total ? `${Math.round(finalizados.length/total*100)}% del total` : "0% del total","✓"],

 

    ["Cancelados",cancelados.length.toLocaleString("es-MX"),total ? `${Math.round(cancelados.length/total*100)}% del total` : "0% del total","×"],

 

    ["En curso",enCurso.length.toLocaleString("es-MX"),"Servicios activos","⌖"],

 

    ["Ingresos",formatearDinero(ingresos),`Ticket prom. ${formatearDinero(ticket)}`,"$"],

 

    ["Llegada / rating",tiempoProm ? `${tiempoProm} min` : "—",ratingProm === "—" ? "Sin calificaciones" : `★ ${ratingProm} promedio`,"◷"]
  ...
  contenedor.innerHTML = kpis.map(([label,value,sub,icon]) => `

 

    <article class="reportKpiCard">

 

      <div class="reportKpiLabel">${escaparHtml(label)}</div>

 

      <div class="reportKpiValue">${escaparHtml(value)}</div>

 

      <div class="reportKpiSub">${escaparHtml(sub)}</div>

 

      <div class="reportKpiIcon">${icon}</div>
  ...
  const tipos = reportAgrupar(servicios,s => s.servicio || "Sin tipo");

 

  const canales = reportAgrupar(servicios,reportCanalServicio);

 

  const estados = reportAgrupar(servicios,s => s.estado || "Sin estado");

 

  renderReportBreakdown("reportChannels",canales,total);

 

  renderReportBreakdown("reportStatuses",estados,total);

 

  reportDibujarTipos("reportTypesChart",tipos);

 

  renderReportProviders(servicios);
  ...
  servicios.forEach(s => {
    ...
    const ms = reportFechaServicio(s);
    ...
    const key = reportFechaLocalInput(f);

 

    dias.set(key,(dias.get(key)||0)+1);
  ...
  const datosDias = [...dias.entries()]
    ...
    .map(([fecha,valor]) => ({
      ...
      label:new Date(`${fecha}T12:00:00`).toLocaleDateString("es-MX",{day:"2-digit",month:"short"})
  ...
  reportDibujarBarras("reportServicesChart",datosDias);

 

  const totalLabel = document.getElementById("reportTotalServicesLabel");

 

  if (totalLabel) totalLabel.textContent = `${total} ${total === 1 ? "servicio" : "servicios"}`;
  ...
  if (periodLabel) periodLabel.textContent = desde && hasta ? `${desde} → ${hasta}` : "Periodo seleccionado";
...
function exportarReporteExcel(){
  ...
  const encabezados = ["Folio","Fecha","Cliente","Canal","Servicio","Proveedor","Estado","Importe"];
  ...
  const filas = servicios.map(s => [
    ...
    reportFechaServicio(s) ? new Date(reportFechaServicio(s)).toLocaleString("es-MX") : "",
  ...
  const csv = "\uFEFF" + [encabezados,...filas].map(f => f.map(escaparCsv).join(",")).join("\r\n");
  ...
  a.download = `reporte-as-click-${reportFechaLocalInput(new Date())}.csv`;
...
function inicializarReportes(){
  ...
  if (!document.getElementById("section-reportes")) return;
  ...
  if (desdeEl && !desdeEl.value) desdeEl.value = reportFechaLocalInput(desde);

 

  if (hastaEl && !hastaEl.value) hastaEl.value = reportFechaLocalInput(hoy);
  ...
  document.querySelectorAll(".reportRangeBtn").forEach(btn => {
    ...
    btn.addEventListener("click",() => aplicarRangoRapidoReportes(btn.dataset.reportRange));
  ...
  [
    ...
    document.getElementById(id)?.addEventListener("input",() => {
      ...
      document.querySelectorAll(".reportRangeBtn").forEach(btn => btn.classList.remove("active"));
  ...
  document.getElementById("reportExportExcelBtn")

 

    ?.addEventListener("click",exportarReporteExcel);

 

  document.getElementById("reportExportPdfBtn")
...
function escucharUbicacionesProveedores(){
  ...
  unsubscribeProviderLocations?.();

 

  unsubscribeProviderLocations = firestoreOnSnapshot(

 

    firestoreCollection(db,"ubicacionesProveedores"),
    ...
    snapshot => {
      ...
      state.providerLocations = new Map(snapshot.docs.map(documento => [documento.id,{id:documento.id,...documento.data()}]));
    ...
    error => console.error("Error leyendo ubicaciones de proveedores:",error)
...
function mostrarResultadosReporteTelefonico(){
  ...
  if (!resultados.length) {
    cont.innerHTML = `
      <div style="padding:10px;border:1px solid #29435c;border-radius:10px;">
        No se encontró cliente por nombre, membresía o placas.
        <div class="cardActions" style="margin-top:10px;">
          <button type="button" class="approve" onclick="capturarClienteSinMembresiaReporteTelefonico()">
            Continuar como cliente sin membresía
          </button>
        </div>
      </div>`;
    return;
...
window.capturarClienteSinMembresiaReporteTelefonico = () => {
  const nombreSugerido = (document.getElementById("rtBusqueda")?.value || "").trim();

  openModal(
    "Cliente sin membresía",
    `
      <p>Captura los datos proporcionados durante la llamada.</p>
      <div style="display:grid;gap:12px;">
        <div>
          <label><b>Nombre</b></label>
          <input id="rtSinMembresiaNombre" type="text" value="${escaparHtml(nombreSugerido)}" style="width:100%;margin-top:6px;">
        </div>
        <div>
          <label><b>Teléfono</b></label>
          <input id="rtSinMembresiaTelefono" type="text" style="width:100%;margin-top:6px;">
        </div>
        <div>
...
window.usarClienteSinMembresiaReporteTelefonico = () => {
  ...
  document.getElementById("rtUidCliente").value = "";
  document.getElementById("rtVehiculoId").value = "";
...
function seleccionarClienteReporteTelefonico(cliente,vehiculo){
  if (!cliente) return;
  const dc = datosClienteReporteTelefonico(cliente);
  const seleccionado = document.getElementById("rtClienteSeleccionado");
  if (seleccionado) {
    delete seleccionado.dataset.sinMembresia;
...
async function crearReporteTelefonico(){
  ...
  const clienteSeleccionadoBox = document.getElementById("rtClienteSeleccionado");
  const clienteSinMembresiaManual = clienteSeleccionadoBox?.dataset.sinMembresia === "true";
  const tipo = document.getElementById("rtTipoServicio")?.value || "";
  const modo = document.getElementById("rtModoAsignacion")?.value || "automatica";
  ...
  const cliente = uidCliente ? state.clients.find(c => c.id === uidCliente) : null;
  const vehiculo = uidCliente
    ? (state.vehicles.find(v => v.uidCliente === uidCliente && v.id === vehiculoId) || null)
    : null;
  if (uidCliente && !cliente) return window.alert("El cliente seleccionado ya no está disponible.");
  ...
  const dc = cliente
    ? datosClienteReporteTelefonico(cliente)
    : {
        uid:"",
        nombre:clienteSeleccionadoBox?.dataset.nombre || "",
        telefono:clienteSeleccionadoBox?.dataset.telefono || "",
        membresia:"",
  ...
  const datosSolicitud = {
    ...
    cliente:{
      ...
      estadoMembresia:dc.estadoMembresia || (cliente?.hasMembership ? "activa" : "sin_membresia")
    },
    uidCliente:cliente?.id || "",
    vehiculo:vehiculo ? {
      id:vehiculo.id,
...
window.limpiarReporteTelefonico = () => {
  ...
  if (cliente) {
    ...
    delete cliente.dataset.sinMembresia;
